...
Cleaned deployment-ready version v3 with robust Gemini JSON parsing.
"""
...
def clean_answer_text(answer: str) -> str:
    """Remove accidental JSON/code-fence artefacts before displaying the answer."""
    ...
    if cleaned.startswith("{"):
        try:
            ...
            loose = extract_answer_fallback(cleaned)
            if loose and not loose.startswith("{"):
                cleaned = loose.strip()

    cleaned = re.sub(r'^\s*"answer"\s*:\s*"', "", cleaned)
    cleaned = cleaned.replace("</div>", "").strip()
    cleaned = re.sub(r'"\s*,?\s*"?brand_mentioned.*$', "", cleaned, flags=re.DOTALL).strip()
...
- answer: a natural answer to the travel query in under 120 words.
...
- evidence: a short phrase from the supplied webpage supporting the answer.
...
with tab1:
    ...
    if not results or "fc_unoptimised" not in results or "fc_faq" not in results:
        ...
        with response_cols[0]:
            ...
            st.text_area(
                "Unoptimised AI response",
                value=unoptimised.get("answer", ""),
                height=170,
                disabled=True,
                label_visibility="collapsed",
                key="unoptimised_answer_display",
            )
        ...
        with response_cols[1]:
            ...
            st.text_area(
                "FAQ optimised AI response",
                value=faq.get("answer", ""),
                height=170,
                disabled=True,
                label_visibility="collapsed",
                key="faq_answer_display",
            )
            if faq.get("evidence"):
