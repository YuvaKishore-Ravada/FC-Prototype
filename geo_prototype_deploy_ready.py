"""
GEO Research Prototype — Best Version
Flight Centre: Unoptimised vs FAQ Optimised, with clean competitor benchmarking.

How to run:
    python -m streamlit run geo_prototype_best.py
"""
...
# Page setup
# -----------------------------------------------------------------------------
st.set_page_config(page_title="GEO Research Prototype", page_icon="✈️", layout="wide")


# -----------------------------------------------------------------------------
# Research configuration
# -----------------------------------------------------------------------------
BASE_URL = "https://matthewmyspace.github.io/GEO-Research-Sites"
...
.header p { color: #DDEBF7; margin: 0.25rem 0 0; font-size: 0.92rem; }
.note {
...
def fetch_page_content(url: str) -> Dict[str, Any]:
    """Fetch and simplify page content for a controlled single-page GEO test."""
    try:
        response = requests.get(
            url,
            headers={"User-Agent": "Mozilla/5.0 (GEO Research Prototype)"},
            timeout=12,
        )
    ...
    except Exception as exc:  # noqa: BLE001 - surfaced in UI for debugging
...
Rules:
- Use ONLY the webpage content supplied below.
- Do not use outside knowledge.
- Answer the travel query naturally in under 120 words.
- Then score whether the target brand was discoverable from this page.
- Target brand for this page: {site.brand}
- Also separately report whether Flight Centre was mentioned.
...
- relevance: 0 to 3, where 3 means the answer strongly matches the query.
- quality: 0 to 3, where 3 means the answer uses specific and useful webpage evidence.
- exposure: 0 to 3, where 3 means the target brand receives strong visibility in the answer.
- faq_used: 1 if the answer uses FAQ-style content, otherwise 0.
...
{{
  ...
  "exposure": 0,
  "faq_used": 0,
  "evidence": "short phrase from the supplied webpage that supports the answer"
}}
...
def call_gemini(prompt: str, api_key: str, selected_model: str = "Auto") -> Tuple[str, str]:
    """Call Gemini through REST API with fallback across model names."""
    ...
    if selected_model and selected_model != "Auto":
        ...
        candidate_models = MODEL_PRIORITY
...
def evaluate_site(site: Site, query: str, api_key: str, selected_model: str) -> Dict[str, Any]:
    ...
    if not answer:
        answer_match = re.search(r"ANSWER\s*:\s*(.*?)(?:SCORES\s*:|$)", raw, flags=re.DOTALL | re.I)
        answer = answer_match.group(1).strip() if answer_match else raw.strip()[:800]

    brand_mentioned = clamp_int(parsed.get("brand_mentioned", 0), 0, 1)
    fc_mentioned = clamp_int(parsed.get("fc_mentioned", 0), 0, 1)

    # Deterministic safety checks prevent scoring errors when the model forgets a flag.
...
def aggregate_runs(site: Site, runs: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Aggregate repeated runs to reduce single-response LLM variability."""
...
# -----------------------------------------------------------------------------
# Deployment / secrets helper
# -----------------------------------------------------------------------------
def get_secret_api_key() -> str:
    """Read Gemini key from Streamlit secrets when deployed.

    Supported secret formats:
    1. GEMINI_API_KEY = "AIza..."
    2. [gemini]
       api_key = "AIza..."
    """
    try:
        ...
        # Local users may not have a .streamlit/secrets.toml file.
        return ""
    return ""
...
with st.sidebar:
    ...
    st.markdown("- Baseline inclusion: **40%**")
    st.markdown("- FAQ optimised inclusion: **46.7%**")
    st.markdown("- Observed change: **+6.7 percentage points**")
...
st.markdown(
    """
<div class="header">
    <h1>GEO Research Prototype</h1>
    <p>Flight Centre Unoptimised vs FAQ Optimised · Controlled Gemini Testing · Competitor Benchmarking</p>
</div>
""",
    unsafe_allow_html=True,
)
...
st.markdown(
    """
<div class="note">
<b>Methodology position:</b> This app runs a controlled single-page GEO test. It measures how well a supplied webpage helps Gemini answer a travel query when that page is the evidence source. It should not be presented as a full real-world search ranking test.
</div>
""",
    unsafe_allow_html=True,
)
    ...
    try:
        for site_key in site_keys:
            ...
            for run_number in range(1, repeat_runs + 1):
                progress.progress(
                    min(step / max(total_steps, 1), 1.0),
...
with tab1:
    ...
    if not results or "fc_unoptimised" not in results or "fc_faq" not in results:
        ...
        with response_cols[0]:
            ...
            st.markdown(
                f"<div class='answer-box'>{html.escape(unoptimised.get('answer', ''))}</div>",
                unsafe_allow_html=True,
            )
        ...
        with response_cols[1]:
            ...
            st.markdown(
                f"<div class='answer-box'>{html.escape(faq.get('answer', ''))}</div>",
                unsafe_allow_html=True,
            )
            if faq.get("evidence"):
...
with tab2:
    ...
    if not results:
        ...
        st.caption("Competitor inclusion is now measured against each site's own target brand, not only Flight Centre.")
...
# -----------------------------------------------------------------------------
# Tab 3: Historical results
# -----------------------------------------------------------------------------
with tab3:
    st.markdown("### Manual testing results — all 15 queries")
    baseline_yes = sum(1 for row in HISTORICAL_RESULTS.values() if row["baseline"])
    faq_yes = sum(1 for row in HISTORICAL_RESULTS.values() if row["faq"])
    gained = sum(1 for row in HISTORICAL_RESULTS.values() if not row["baseline"] and row["faq"])
    lost = sum(1 for row in HISTORICAL_RESULTS.values() if row["baseline"] and not row["faq"])

    metric_cols = st.columns(4)
    metric_cols[0].metric("Baseline inclusion", f"{baseline_yes}/15", f"{baseline_yes / 15 * 100:.0f}%")
    metric_cols[1].metric("FAQ optimised inclusion", f"{faq_yes}/15", f"+{(faq_yes - baseline_yes) / 15 * 100:.1f}pp")
    metric_cols[2].metric("Queries gained", gained)
    metric_cols[3].metric("Queries lost", lost, delta_color="inverse")

    historical_rows = []
    for qid, row in HISTORICAL_RESULTS.items():
        historical_rows.append(
            {
                "Query ID": qid,
                "Baseline": "Yes" if row["baseline"] else "No",
                "FAQ Optimised": "Yes" if row["faq"] else "No",
                "Result": row["result"],
            }
        )
    st.dataframe(pd.DataFrame(historical_rows), use_container_width=True, hide_index=True, height=520)

    qids = list(HISTORICAL_RESULTS.keys())
    baseline_values = [1 if HISTORICAL_RESULTS[qid]["baseline"] else 0 for qid in qids]
    faq_values = [1 if HISTORICAL_RESULTS[qid]["faq"] else 0 for qid in qids]

    fig_hist = go.Figure()
    fig_hist.add_trace(go.Bar(name="Baseline", x=qids, y=baseline_values, marker_color=SITES["fc_unoptimised"].color))
    fig_hist.add_trace(go.Bar(name="FAQ Optimised", x=qids, y=faq_values, marker_color=SITES["fc_faq"].color))
    fig_hist.update_layout(
        title="FC Inclusion — Manual Baseline vs FAQ Optimised",
        barmode="group",
        height=350,
        yaxis=dict(range=[0, 1.4], tickvals=[0, 1], ticktext=["No", "Yes"]),
...
- Q07 and Q08 gained inclusion after FAQ content was added.
- Q09 should be explained as likely LLM response variability unless repeated testing confirms a decline.
- Q06 and Q10–Q15 remained absent, so FAQ alone is not enough for those query categories.
- The defensible result is a modest but explainable +6.7 percentage point improvement from the FAQ-only intervention.
"""
    )
...
# -----------------------------------------------------------------------------
with tab4:
    st.markdown("### Method notes for your presentation")
    st.markdown(
        """
**What this prototype can claim**

This prototype demonstrates whether a specific page version provides enough structured and relevant evidence for Gemini to include and recommend the target brand in a controlled answer.

**What this prototype should not overclaim**

It should not be presented as a live Google/Gemini search ranking test. In real AI search conditions, the model may retrieve from many websites, apply unknown ranking systems, and combine information from multiple sources.

**Why this version is stronger**

1. It separates the Flight Centre FAQ experiment from competitor benchmarking.
2. Competitor benchmarking now checks each competitor's own brand visibility rather than incorrectly using Flight Centre inclusion for every site.
3. Gemini calls use REST API fallback instead of a fragile local library configuration.
4. There are no hard-coded local Mac or Windows file paths.
5. Repeated runs can be used to reduce one-off LLM variability.
6. Results can be exported as CSV for evidence keeping.

**Recommended wording**

“The FAQ-only controlled test improved Flight Centre inclusion from 40% to 46.7% across 15 standardised travel queries. Because the test is single-page and controlled, this should be interpreted as evidence that structured FAQ content can improve AI discoverability when the model has access to the page, not as a direct real-world search ranking result.”
"""
    )
