...
Cleaned deployment-ready version v8 with master consensus research results.
"""
...
with tab1:
    if st.session_state.get("manual_mode"):
        qid = st.session_state.get("manual_query_id", "Q01")
...
with tab2:
    results = st.session_state.get("geo_results")
    if st.session_state.get("manual_mode"):
        st.info("Benchmarking uses live Gemini testing. Switch Test scope to 'All sites' and run the test to view competitor benchmarking.")
    elif not results:
