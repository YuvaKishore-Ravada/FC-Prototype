...
Cleaned deployment-ready version v10 with presentation-grade visual dashboard.
"""
...
.hero {
    background:
        radial-gradient(circle at 10% 15%, rgba(255,255,255,0.18), transparent 22%),
        linear-gradient(135deg, #123E67 0%, #1F6AA5 52%, #2E75B6 100%);
    padding: 2rem 2.2rem;
    border-radius: 22px;
    margin-bottom: 1.15rem;
    box-shadow: 0 16px 38px rgba(18,62,103,0.22);
...
.result-card.red { border-top: 6px solid #E57373; }
.result-card.green { border-top: 6px solid #66BB6A; }
.result-title {
    font-size: 1.1rem;
    font-weight: 800;
    color: var(--ink);
...
.consensus-pill.included {
    background: #E8F7EE;
    color: #167A3B;
    border: 1px solid #B8E6C8;
}
.consensus-pill.not-included {
    background: #FFF1F0;
    color: #B42318;
...
.metric-card h4 { margin: 0 0 0.45rem; color: #64748B; font-size: 0.78rem; text-transform: uppercase; letter-spacing: .06em; }
...
.kpi-card.blue { border-top: 6px solid #2E75B6; }
.kpi-card.green { border-top: 6px solid #2EAD5B; }
.kpi-card.gold { border-top: 6px solid #F4B942; }
.kpi-card.red { border-top: 6px solid #E74C3C; }
...
with st.sidebar:
    st.markdown("### ✈️ Flight Centre GEO")
...
st.markdown(
    """
<div class="hero">
    <div class="hero-eyebrow">QUT x Flight Centre · GEO Research Dashboard</div>
    <h1>Flight Centre GEO Research Prototype</h1>
    <p>A controlled research dashboard measuring whether FAQ-only optimisation improves Flight Centre visibility in AI-generated travel responses.</p>
    <div class="hero-badges">
        <span class="badge">✓ Master consensus logic</span>
        <span class="badge">✓ 15 standardised queries</span>
        <span class="badge">✓ 3 validation runs</span>
        <span class="badge">✓ Live benchmark mode</span>
    </div>
</div>
""",
    unsafe_allow_html=True,
...
st.markdown(f"<div class='note'><b>Current query:</b> <code>{html.escape(query_text)}</code></div>", unsafe_allow_html=True)
    ...
    if query_mode == "Standard Q01–Q15" and run_scope == "FC only":
        ...
        if not master_result:
            ...
            with response_cols[0]:
                base_status_class = "included" if master_result["baseline_consensus"] == "Included" else "not-included"
                st.markdown(
                    f"""
<div class="result-card red">
    <div class="result-title">🔴 FC Unoptimised</div>
    <div class="consensus-pill {base_status_class}">Master consensus: {html.escape(master_status_text(master_result['baseline_consensus']))}</div>
    <div class="answer-box">{html.escape(master_result_message(master_result, 'baseline'))}</div>
</div>
""",
                    unsafe_allow_html=True,
                )
            ...
            with response_cols[1]:
                faq_status_class = "included" if master_result["faq_consensus"] == "Included" else "not-included"
                st.markdown(
                    f"""
<div class="result-card green">
    <div class="result-title">🟢 FC FAQ Optimised</div>
    <div class="consensus-pill {faq_status_class}">Master consensus: {html.escape(master_status_text(master_result['faq_consensus']))}</div>
    <div class="answer-box">{html.escape(master_result_message(master_result, 'faq'))}</div>
</div>
""",
                    unsafe_allow_html=True,
                )
            ...
            kpi_cols[0].markdown(f"<div class='kpi-card blue'><div class='kpi-label'>Baseline evidence</div><div class='kpi-value'>{baseline_yes}/3</div><div class='kpi-sub'>{baseline_yes / 3 * 100:.1f}% validation support</div></div>", unsafe_allow_html=True)
            kpi_cols[1].markdown(f"<div class='kpi-card green'><div class='kpi-label'>FAQ optimised evidence</div><div class='kpi-value'>{faq_yes}/3</div><div class='kpi-sub'>{faq_yes / 3 * 100:.1f}% validation support</div></div>", unsafe_allow_html=True)
            kpi_cols[2].markdown(f"<div class='kpi-card gold'><div class='kpi-label'>Vote change</div><div class='kpi-value'>{vote_change:+d}</div><div class='kpi-sub'>change across validation runs</div></div>", unsafe_allow_html=True)
            kpi_cols[3].markdown(f"<div class='kpi-card blue'><div class='kpi-label'>Master result</div><div class='kpi-value' style='font-size:1.45rem'>{html.escape(master_result['master_result'])}</div><div class='kpi-sub'>majority-rule outcome</div></div>", unsafe_allow_html=True)
            kpi_cols[4].markdown(f"<div class='kpi-card green'><div class='kpi-label'>FAQ evidence</div><div class='kpi-value'>{faq_evidence}/3</div><div class='kpi-sub'>FAQ signal detected</div></div>", unsafe_allow_html=True)
            ...
            fig.update_layout(
                title="Evidence Strength for Selected Query",
                barmode="group",
                height=390,
...
with tab3:
    ...
    metric_cols[0].markdown(f"<div class='kpi-card blue'><div class='kpi-label'>Consensus baseline</div><div class='kpi-value'>{baseline_consensus_total}/15</div><div class='kpi-sub'>{baseline_consensus_total / total_queries * 100:.1f}% of queries</div></div>", unsafe_allow_html=True)
    metric_cols[1].markdown(f"<div class='kpi-card green'><div class='kpi-label'>Consensus FAQ optimised</div><div class='kpi-value'>{faq_consensus_total}/15</div><div class='kpi-sub'>{faq_consensus_total / total_queries * 100:.1f}% of queries</div></div>", unsafe_allow_html=True)
    metric_cols[2].markdown(f"<div class='kpi-card gold'><div class='kpi-label'>Consensus change</div><div class='kpi-value'>{(faq_consensus_total - baseline_consensus_total) / total_queries * 100:+.1f}pp</div><div class='kpi-sub'>majority-rule improvement</div></div>", unsafe_allow_html=True)
    metric_cols[3].markdown(f"<div class='kpi-card gold'><div class='kpi-label'>Vote-weighted change</div><div class='kpi-value'>{(faq_vote_total - baseline_vote_total) / total_runs * 100:+.1f}pp</div><div class='kpi-sub'>all validation runs</div></div>", unsafe_allow_html=True)
    metric_cols[4].markdown(f"<div class='kpi-card green'><div class='kpi-label'>FAQ evidence</div><div class='kpi-value'>{faq_evidence_total}/{total_runs}</div><div class='kpi-sub'>FAQ signal references</div></div>", unsafe_allow_html=True)
